...
$('.cg-content-variation input').on('click', function() {
    var cgColHold = '',
        cgSizHold = '';
    ...
    if ($(this).attr('value') == 'white') {
        ...
        // Switch variations
        $('.test-code .at-compare-table').removeAttr('id');
        // Screen image
        $('.pzn-v-btm img').attr('src', 'https://images.samsung.com/is/image/samsung/p5/uk/pzn/canvas/comparison-grid/Screen-icon-black.png?$ORIGIN_PNG$');
        // Switch variations Image
        for (var v = 0; v < $('.test-code .at-compare-table').length; v++) {
            cgColHold = $('.at-compare-table .cld-intro-img img').eq(v).attr('src').replace('-black', '-white');
            $('.at-compare-table .cld-intro-img img').eq(v).attr('src', cgColHold);
        };

    } else {
        // Switch variations
        $('.test-code .at-compare-table').attr('id', 'pzn-blk-bg');
        // Screen image
        $('.pzn-v-btm img').attr('src', 'https://images.samsung.com/is/image/samsung/p5/uk/pzn/canvas/comparison-grid/Screen-icon-white.png?$ORIGIN_PNG$');

        for (var v = 0; v < $('.test-code .at-compare-table').length; v++) {
            cgColHold = $('.at-compare-table .cld-intro-img img').eq(v).attr('src').replace('-white', '-black');
            $('.at-compare-table .cld-intro-img img').eq(v).attr('src', cgColHold);
        };
...
window.onload = function() {
    document.getElementById("devicehandle").value = "s20_compare";
    optionControl();
}
...
function optionControl() {
    ...
    } else if (jQuery("option:checked").val() == "s20_compare") {
        jQuery(".test-code .s20").css("display", "block");
    } else if (jQuery("option:checked").val() == "s205g_compare") {
        jQuery(".test-code .s205g").css("display", "block");
    } else if (jQuery("option:checked").val() == "s20plus_compare") {
        jQuery(".test-code .s20plus").css("display", "block");
    } else if (jQuery("option:checked").val() == "s20ultra_compare") {
        jQuery(".test-code .s20ultra").css("display", "block");
    ...
    } else if (jQuery("option:checked").val() == "mate20pro_compare") {
        jQuery(".test-code .mate20pro").css("display", "block");
    } else if (jQuery("option:checked").val() == "p20pro_compare") {
        jQuery(".test-code .p20pro").css("display", "block");
    } else if (jQuery("option:checked").val() == "p30pro_compare") {
        jQuery(".test-code .p30pro").css("display", "block");
    } else if (jQuery("option:checked").val() == "honor9_compare") {
        jQuery(".test-code .honor9").css("display", "block");
    };
};
    ...
    jQuery(".editLink-container .save-btn").on("click", function() {
        ...
        for (var v = 0; v < jQuery(".test-code .upg-btn-wrap a").length; v++) {
            ...
            if (jQuery(".editLink-container .get-text").val().length > 0) {
                jQuery(".test-code .upg-btn-wrap a").eq(v).find('span').text(jQuery(".editLink-container .get-text").val().replace(/[']/g, "’").replace(/["]/g, "“"));
            };
            ...
            if (jQuery(".test-code .upg-btn-wrap a").eq(v).attr("data-aa-evar-desc") == undefined && jQuery(".editLink-container .get-evar-desc").val().length < 1) {
                ...
                jQuery(".test-code .upg-btn-wrap a").eq(v).attr("data-aa-evar-desc", jQuery(".editLink-container .get-evar-desc").val().replace(/[']/g, "’").replace(/["]/g, "“"));
        ...
        if (jQuery(".test-code .upg-btn-wrap a").eq(toolTipPosition).attr("data-aa-evar-desc").length > 0 && jQuery(".editLink-container .get-evar-desc").val().length > 0) {
            jQuery(".test-code .upg-btn-wrap a").eq(toolTipPosition).attr("data-aa-evar-desc", jQuery(".editLink-container .get-evar-desc").val().replace(/[']/g, "’").replace(/["]/g, "“"));
        };
    ...
    jQuery(".tracking-container .save-btn").on("click", function() {
        ...
        for (var a = 0; a < jQuery(".accordion_container").length; a++) {
            ...
            if (jQuery(".tracking-container .get-text").val().length > 0) {
                jQuery(".accordion_container").eq(a).find('.accordion_head').eq(copyPoz).find('.pzn-sec-title').text(jQuery(".tracking-container .get-text").val().replace(/[']/g, "’").replace(/["]/g, "“"));
            };
            ...
            if (jQuery(".tracking-container .get-evar-desc").val().length < 1 && jQuery(".accordion_container").eq(a).find('.accordion_head').eq(copyPoz).attr('data-aa-evar-desc') == undefined) {
                ...
                jQuery(".accordion_container").eq(a).find('.accordion_head').eq(copyPoz).attr('data-aa-evar-desc', jQuery(".tracking-container .get-evar-desc").val().replace(/[']/g, "’").replace(/["]/g, "“") + ':' + jQuery(".accordion_container").eq(a).find('.accordion_head').eq(copyPoz).find('.pzn-sec-title').text().toLowerCase());
            };
...
var s = `<style type="text/css">
      /*UPDATE*/
      .accordion_body td img {
            margin: 0 auto!important;
        }
  .pznGridInside .m_content-compare {
    display: none!important;
      ...
      #pzn-blk-bg.at-compare-table .cld-intro-txt h5 {
            color: #fff;
        }
        #pzn-blk-bg .accordion_head {
            color: #fff;
        }
      #pzn-blk-bg .accordion_head span:first-child {
          border: 1px solid #fff;
        }
      #pzn-blk-bg .accordion_body p {
        color: #fff;
      }
      #pzn-blk-bg .accordion_container table {
        color:#fff;
      }
      #pzn-blk-bg .at-compare-table small {
          color: #fff!important;
      }
        #pzn-blk-bg .accordion_container .upg-btn-wrap span {
        border: 1px solid #fff;
        color: #fff;
      }
    #pzn-blk-bg .accordion_container td:nth-of-type(2),
    #pzn-blk-bg .accordion_container td:nth-of-type(3) {
        border-left: 1px solid rgba(255,255,255,0.15);
      }
      #pzn-blk-bg .accordion_head {
        border-top: 1px solid rgba(255,255,255,0.5);
      }
    #pzn-blk-bg .at-compare-table .pzn-width-one img,
    #pzn-blk-bg .at-compare-table .pzn-width-two img,
      #pzn-blk-bg .at-compare-table .pzn-width-three img {
        -webkit-filter:invert(100%);
         filter:progid:DXImageTransform.Microsoft.BasicImage(invert='1');
      }
      #pzn-blk-bg .accordion_container .accordion_head:first-child {
        ...
        #pzn-blk-bg.at-compare-table .pzn-screen-size span {
          background: #000;
      ...
      /* Open inside modal ===============================================*/
      .layer-models .m_ly_inner > section:nth-of-type(2) {
            display: none!important;
        }
        .m_ly_inner .at-compare-table > article {
...
function submission() {
    ...
    var changeAllThis = ".test-code h2, .test-code a span, .test-code p, .test-code span, .test-code strong, .test-code h1, .test-code h3, .test-code h5";
    ...
    if (jQuery("option:checked").val() == "s9_compare") {
        ...
        cleanHTML = jQuery(".test-code .s20").html();
    } else if (jQuery("option:checked").val() == "s205g_compare") {
        cleanHTML = jQuery(".test-code .s205g").html();
    } else if (jQuery("option:checked").val() == "s20plus_compare") {
        cleanHTML = jQuery(".test-code .s20plus").html();
    } else if (jQuery("option:checked").val() == "s20ultra_compare") {
        cleanHTML = jQuery(".test-code .s20ultra").html();
    } else if (jQuery("option:checked").val() == "note8_compare") {
        cleanHTML = jQuery(".test-code .n8").html();
    } else if (jQuery("option:checked").val() == "note9_compare") {
        ...
        cleanHTML = jQuery(".test-code .mate20pro").html();
    } else if (jQuery("option:checked").val() == "p20pro_compare") {
        cleanHTML = jQuery(".test-code .p20pro").html();
    } else if (jQuery("option:checked").val() == "p30pro_compare") {
        cleanHTML = jQuery(".test-code .p30pro").html();
    } else if (jQuery("option:checked").val() == "honor9_compare") {
        cleanHTML = jQuery(".test-code .honor9").html();
    };
    ...
    var c = `<script type="text/javascript">
            ...
            function sendBuyData(data) {
              ...
              s.linkTrackEvents ="event` + getEvent + `";
            ...
            function sendSecData(data) {
              ...
              s.linkTrackEvents ="event` + getSecEvent + `";
            ...
            function holdComparison(){
                function allForComparison(){
                    jQuery('` + cleanHTML.trim() + `').insertBefore('#design');
                ...
                var jqueryCGcheck = setInterval(function() {
                    if (window.$){
                        clearInterval(jqueryCGcheck);
                        allForComparison();
                    }
                }, 100);
            }
    ...
    var q = `<script type="text/javascript">
            ...
            function holdComparison(){
                ...
                function pznGridAppend(){

                    if ($('.at-compare-table').length <= 0){
                        $('.m_ly_outer .m_ly_inner').addClass('pznGridInside');
                                  // HTML
                                jQuery('` + cleanHTML.trim() + `').insertBefore('.m_content-compare');
                            ...
                            $('.c_btn_canvas.black').eq(0).on('click', function(){
                                var pznAppendGrid = setInterval(pznGridAppend, 50);
                                setTimeout(function(){
                                    clearInterval(pznAppendGrid);
                                    console.log('stopped');
                                }, 900);
                            });
                            };
